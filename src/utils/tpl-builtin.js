...
export const filters = {
    ...
    boolean: (input) => JSON.parse(input),
    ...
    fromNow: (input, inputFormat = '') =>
        moment(input, inputFormat).fromNow(),
    dateModify: (
        input,
        modifier = "add", // 'add' | 'subtract' | 'endOf' | 'startOf'
        amount = 0,
        unit = 'days'
    ) => {
    ...
    date: (input, format = 'YYYY-MM-DD HH:mm:ss', inputFormat = 'X') =>
...
export const tokenize = (
    ...
    return resolveVariableAndFilter(str, data, defaultFilter) ?? '';
};
        ...
        if (typeof ignoreFunction === 'function' && ignoreFunction(key, value)) {
            ...
            const v =
                isPlainObject(value) &&
                    ...
                    ? from[keys[0].substring(1)].map((raw) =>
                        dataMapping(
                            value[keys[0]],
...
export function register() {
    return {
        ...
        removeEscapeToken: (str) =>
            typeof str === 'string' ? str.replace(/\\\$/g, '$') : str,
        compile: (str, data, defaultFilter = '| html') =>
